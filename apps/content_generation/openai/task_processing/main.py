...
class TaskProcessing:
    ...
    def detailed_analysis_composition(self):
        ...
        for step_number in range(task_steps_count):
            ...
            # Элементные шаги больше не делают итоговый вывод — он вынесен
            # в отдельный финальный шаг ниже.
            step_data["Шаг задачи последний или нет"] = False
        ...
        # Отдельный финальный шаг — только итоговый вывод по ПОЛНОМУ составу.
        # Раньше вывод запрашивался на последнем элементном шаге и конкурировал
        # с поэлементным разбором: при коротком остатке состава модель иногда
        # возвращала пустой result. Выделенный запрос без элементов это чинит.
        conclusion_step = deepcopy(self.collection_data)
        conclusion_step["Элементы состава для шага задачи"] = []
        conclusion_step["Номер шага задачи"] = task_steps_count + 1
        conclusion_step["Шаг задачи последний или нет"] = True
        conclusion_step["Смещение для нумерации"] = total_count + 1
        task_steps.append(conclusion_step)
