...
# Пул прокси: выдаёт живой прокси, через который fetcher идёт в сеть.
#
# Как работает get():
...
# fetcher зовёт mark_bad(proxy) — прокси выкидывается, следующий get() даст другой.
...
class ProxyPool:
    ...
    def mark_bad(self, proxy: Proxy) -> None:
        ...
        и fetcher (сайт не пустил через этот прокси).
